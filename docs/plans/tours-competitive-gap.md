...
## 3.5 다국어 기사 매칭 갭 (v3 amendment, 2026-04-25)

| # | 항목 | 현재 상태 | 영향 |
|---|---|---|---|
| **L1** | 기사 가능 언어 표기 | `pricing_spec.json` `vehicles.staria.features_ko[3]: "영어 소통 가능 기사"` 일반 문구만. 일·중 언급 없음 | 일/중 고객 의심 |
| **L2** | 사용자 필터 (영어 / 일본어 / 중국어) | 없음 | 일/중 시장 미공략 |
| **L3** | 기사별 프로필 (사진/언어/경력/리뷰) | 없음 | VIP 신뢰 약화 |
| **L4** | 예약 시 언어 선택 → 가용 기사 자동 매칭 | 없음 (수동 매칭 추정) | 운영 부담 |

**해결안 (P0-가에 묶음):**
- `Tour` 데이터에 `driverLanguages: ('en'|'ja'|'zh')[]` 배열 (대부분 ['en']로 시작)
- ToursPage 필터에 언어 멀티체크박스 추가 (기존 region/duration 옆)
- TourDetailPage 메타 칩에 "EN · JA available" 표시
- 예약 모달(P0-가1)에 언어 선택 → 가능 기사 매칭 안내 ("일본어 기사는 +₩50,000 옵션" 같은 추가요금 정책 데이터화)
- Driver 별도 모델은 P2-D로 미루고 P0-가에는 Tour-level flag만

## 9. 가격 시장 비교 (v3 amendment) — **시장 대비 너무 저렴**

### 9.1 현재 CocoTrip 가격 ([pricing_spec.json](src/data/pricing_spec.json), [tours.ts](src/data/tours.ts))

| 상품 | CocoTrip 가격 (Staria 8인 1팀) | USD 환산 | 인당(2인 기준) | 인당(8인 기준) |
|---|---|---|---|---|
| 서울 시티투어 1일 (8h) | ₩330,000 (data: $208) | $235 | $117 | **$26** |
| 서울 근교/한강 1일 | ₩343,200 | $245 | $122 | $30 |
| DMZ 1일 | ₩343,200 | $245 | $122 | $30 |
| 강원도 1일 | ₩436,800 | $312 | $156 | $39 |
| 부산 1일 | ₩572,000 | $408 | $204 | $51 |
| 경주·전주 1일 | ₩468,000 | $334 | $167 | $42 |
| ICN→서울도심 공항픽업 | ₩124,800 | $89 | — | — |
| ICN→부산 공항픽업 | ₩600,000 | $429 | — | — |

### 9.2 시장 비교 (2026-04 Klook/Viator/Trazy/OnedayKorea/koreatraveleasy)

| 상품 | 시장 일반 가격 (per group, USD) | CocoTrip | 상태 |
|---|---|---|---|
| 서울 1일 프라이빗 (van + 영어기사) | **$472-668** (OnedayKorea/Trazy 평균), Tripadvisor private daily $250-700 | **$235** | 🔴 시장 대비 50-65% 저렴 |
| DMZ 프라이빗 1일 | $440-1100 (4인 기준 $110/인 × 4) | $245 | 🔴 약 2배 저렴 |
| 부산 1일 프라이빗 | $410-450 | $408 | 🟡 시장 평균과 비슷 |
| 제주 1일 프라이빗 | $470-500 | (별도 데이터 필요) | — |
| 다일 패키지 (9일) | per person $5,355+ (4스타 호텔 포함) | 우리 multi_day는 인터시티 룰만 있음 | 🔴 패키지 부재 |

### 9.3 결론 + 권고

**합리적이라기보다 "너무 싸다"** — VIP 프라이빗 포지셔닝과 가격이 모순.

**원인 추정:**
- charter pricing이 한국 내수 가격 기준으로 책정 (운송 원가 베이스)
- 외국인 시장의 인지 가치(perceived value)와 별개
- 경쟁사는 "투어 + 가이드 콘텐츠" 가격을 추가, CocoTrip은 "차량 + 기사"만 가격에 반영

**권고 (3가지 옵션):**

#### 옵션 V1 — 가격 일괄 +30-40% 인상 (가장 단순)
| 상품 | 현재 | 권고 | 시장 대비 |
|---|---|---|---|
| 서울 1일 | $235 | **$310-340** (₩430k-470k) | 약 2/3 시장 평균, 여전히 경쟁력 |
| DMZ 1일 | $245 | **$340-380** | 시장 하단 |
| 부산 1일 | $408 | **$520** | 시장 상단, VIP 포지션 |
| 강원도 1일 | $312 | **$420** | 시장 평균 |
| 제주 1일 | (신규) | **$450-490** | 시장 평균 |

#### 옵션 V2 — Tier 가격제 (Standard / Premium / VIP)
- Standard = 현재 가격 (Staria 영어 기사만)
- Premium = +25% (영어/일/중 기사 선택, 사진사 옵션, 미슐랭 점심 추천)
- VIP = +60% (Sprinter, 가이드 동행, 한복 대여 포함, hotel 픽업 우선)

#### 옵션 V3 — 가격 유지 + 객단가 add-on 강화 (가장 안전)
- 베이스 가격 그대로 유지 (시장 진입 가격)
- Add-on (한복 ₩30k, 입장권 통합 ₩50k, 사진사 ₩150k, 일/중 기사 +₩80k, 미슐랭 점심 +₩100k/인)을 **결제 직전 강제 노출**
- 평균 객단가 +30-50% 가능, 신규 고객 가격 거부감 회피

### 9.4 권고
**V3 → V1 단계 진행** 권장. 초기엔 add-on 활성화로 객단가 인상 + 데이터 수집 (어떤 add-on이 잘 팔리는지 1-2개월). 이후 가격 인상 V1을 적용해 VIP 포지셔닝 강화. V2는 운영 부담 큼 (3 tier 데이터·UI·운영자 학습).

영향 파일 (가격 인상 시):
- [src/data/pricing_spec.json](src/data/pricing_spec.json) — 단일 SSOT 갱신만
- [scripts/sync-pricing.js](scripts/sync-pricing.js) 실행 → [api/_pricing_spec.json](api/_pricing_spec.json) 자동 동기화
- [src/data/tours.ts](src/data/tours.ts) priceFrom 필드도 SSOT 참조하도록 변환 (현재 하드코딩)
