...
- 구 `TEST-` prefix 경로는 2026-05-07 (이슈 17) 에 `ADMIN-BYPASS-` 로 교체됐다. `TEST-` 경로는 **2026-07-20 완전 폐지** — 환경·env 무관 항상 403. 그 게이트를 열던 `BRAINTREE_ENV` 는 Vercel 에 등록된 적조차 없어 이미 상시 거부 상태였고, admin 검사가 없는 무인증 우회라 경로를 제거했다.
