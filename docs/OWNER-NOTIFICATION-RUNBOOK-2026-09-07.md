...
| Vercel Production Secret | `OWNER_NOTIFICATION_CURSOR_SECRET` | 알림 기록 전용 암호화 키. 암호학적 난수 32바이트 이상을 생성해 저장. 서버 전용이며 Preview/Development/VITE_/Git에 넣지 않음 |
| Vercel 기존 | `CRON_SECRET` | 기존 예약 작업 인증은 그대로 유지. 전용 암호화 키가 없을 때만 이전 배포와 호환하기 위해 cursor 키로 사용하며, 이 경우 최소 32바이트 검증 유지 |
...
- cursor 암호화는 `OWNER_NOTIFICATION_CURSOR_SECRET`에서 HKDF-SHA256으로 목적을 구분해 파생한 AES-256-GCM 키를 쓴다. 전용 키가 미설정(undefined)인 경우에만 이전 배포와 호환하도록 기존 `CRON_SECRET`을 사용한다. 전용 키의 명시된 빈 값/공백/형식 오류/길이 부족은 기존 키로 우회하지 않는다. 최소 32바이트 조건을 유지하며 매번 임의의 96비트 IV를 만들고 설정 범위와 원본 종류를 인증 추가 데이터에 묶는다. 앱은 키를 발급하거나 파일/로그에 저장하지 않는다.
- 암호문 변조·범위 불일치·암호 해제 실패·실제 사용 중인 암호화 키 교체 시 자동 초기화/소급 발송 없이 보류한다. 전용 키를 쓰는 경우 공용 `CRON_SECRET` 변경은 cursor 암호화에 영향을 주지 않지만 cron 인증에는 별도 영향을 준다. 이미 제어 문서가 있는 환경에서 전용 키로 전환하려면 기준 시각을 보존하는 별도 승인 이관 계획이 필요하며, 제어 문서를 지우거나 과거 문의를 자동 재발송하지 않는다.
