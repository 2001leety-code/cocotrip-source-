...
- `tests/unit/lint-charter-distance-lifecycle.test.tsx`는 입력 전환·늦은 응답·수동 거리·좌표·실패 캐시를 외부 fetch 응답만 대체해 검사한다. 실제 문의 저장/발송이나 결제 검증을 뜻하지 않는다.

## 2026-09-21 주소 검색의 이전 응답 재등장

- 실제 `AddressAutocomplete` 검사에서 검색 중 입력 삭제 후 이전 결과가 다시 나타나고, 느린 이전 검색이 최신 결과를 덮는 두 경우를 재현했다. 요청 취소와 응답 무효화로 보완했다.
- `tests/unit/lint-address-lifecycle.component.test.tsx`는 삭제·응답 역전·좌표 확정/부모 초기화·서버 오류 후 재검색을 검사한다. 외부 검색 응답만 대체하며, 실제 문의 저장·발송 검증을 의미하지 않는다.
