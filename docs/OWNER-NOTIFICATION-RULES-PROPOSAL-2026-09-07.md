# 기기 알림 조회 규칙: 승인된 최소 수정안

## 결론과 승인 범위

알림 등록 상태 확인을 막는 운영 보안 규칙 결함을 확인했다. 운영자는 2026-09-07
이 제안의 수정·배포 질문에 "응 진행해"로 명시 승인했다.
승인 범위는 **본인 계정의 알림 기기 문서 단건 조회 허용**이다.
...
## 승인 후 진행 순서와 배포 범위 보호

1. 운영 규칙과 원격 main의 변경 여부를 다시 확인한다. 전체 규칙과 로컬 차이가 있으면 덮어쓰지 않는다.
   2026-09-07 재확인 결과 로컬에만 기존 주석 1줄과 명시적 read/write 거부 규칙 2줄이 있었다.
   권한을 여는 차이는 아니지만 이번 승인에 섞지 않는다. 배포 후보는 현재 운영 원문에 위 GET 조건만
   추가하여 만들고, 나머지 원문이 그대로인지 검사한다. 일반 전체 파일 배포로 이 차이까지 밀어 넣지 않는다.
2. 최소 조건과 가상 회귀검사를 추가하고 전체 규칙 문법·동작, 웹 빌드·전체 회귀·린트를 검증한다.
3. 준비된 알림 UI와 하나의 브랜치/PR로 묶어 푸시·검사·머지한다. 보안 규칙은 Vercel 웹 배포와
   별도다. Firebase 공식 Rules API로 운영 원문 기반 후보를 등록한 뒤 해당 Firestore release만 갱신한다.
   기존 ruleset 이름과 전체 원문 해시가 검사 당시와 같은지 직전에 재확인하고 달라지면 중단한다.
   기존 ruleset은 삭제하지 않아 되돌릴 근거를 보존한다. Storage·인덱스·다른 권한은 배포하지 않는다.
...
위 40건은 최초 격리 검사 기록이다. 전체 운영 규칙 후보의 추가 검증과 실제 배포 결과는 별도 기록한다.
실제 기기 등록과 수신은 별도 검증 항목이다.
예약·문의·수신 메일·API 비용 자동 발송 연결도 별도 후속 작업이며 이 규칙 수정만으로 완성되지 않는다.

## 승인 후 전체 원문 검증 결과

운영 기존/운영 후보/로컬 기존/로컬 후보를 각각 전체 규칙 파일로 컴파일하여 28개 가상 요청씩
총 112건이 예상과 일치했다. 문법 오류 0, 서비스 함수 호출 0이다.
본인 기존/없는 문서, 잘못된 저장 UID 타입, 밑줄·정규식 문자 UID, UID 접두어 혼동,
목록·삭제 거부, 기존 생성·수정 조건 유지까지 포함한다.

- 현재 운영 원문: `e4d41f9cde8a6bae339e29636c5513da31d50495f2aea40cfac81bd92e1744ec`.
- 승인된 GET만 추가한 운영 후보: `e33f2c29d80aa740006e2a8b51b9f1af1c6c426c7b7745963301ad33806d3895`.
- 운영 후보에서 추가 GET을 제거하면 기존 운영 원문과 바이트 단위로 같아야 한다.
- 로컬에만 있던 명시적 거부 2줄과 주석 1줄은 이 운영 후보에 포함하지 않는다.

재현 명령(로컬 PC, 이미 설치·로그인된 Firebase CLI 사용):

```text
node scripts/verify-owner-notification-rules.mjs
npx vitest run tests/unit/owner-notification-rules.test.ts
node scripts/verify-owner-notification-rules.mjs --simulate --firebase-tools-root="<npm root -g 결과>/firebase-tools" --expected-ruleset="<실제 배포된 ruleset 전체 이름>"
```

첫 두 명령은 인증 없는 소스 계약 검사이며, Firebase의 실제 권한 판정을 흉내 내어 통과시키지 않는다.
마지막 명령만 운영 규칙을 읽고 공식 `projects:test`에 가상 요청을 보낸다. 규칙 배포·고객 문서
조회·사용자 생성 기능은 없다. 배포 후에는 새 ruleset 이름을 명시하고 같은 검사를 반복한다.
