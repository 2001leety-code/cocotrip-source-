...
- 새 API 건강검사에서 저장소의 기존 관성대로 `actions/checkout@v4`와
  `actions/setup-node@v4`를 사용해 실제 실행은 성공했지만 Node 20 지원 종료 경고가
  남았다. 공식 최신 안정판이 둘 다 v7임을 확인한 뒤 이 워크플로만 v7로 올리고 실제
  수동 실행으로 다시 검증한다. 다른 워크플로의 일괄 메이저 변경은 별도 호환성 감사
  없이 기계적으로 확대하지 않는다.
