...
// `/planner` 정적 본문(색인용) + 모바일 라이트 셸 글자색 잠금 — 2026-08-06.
//
// 왜 이 스펙이 있나
...
//   ② 그 본문은 데스크톱(어두운 배경) 기준 흰 글씨인데 **모바일 위저드만 라이트 셸**이라
//      흰 배경에 흰 글씨가 됐다. 같은 페이지의 `AiPlannerPricingNote` 도 같은 이유로
//      `$9.90` 이 안 보이는 상태였다 — "화면에 금액이 없다"를 고치려고 만든 컴포넌트가
//      모바일에서만 그 문제를 그대로 갖고 있었다.
//
...
test.describe('/planner 색인용 본문', () => {
  ...
  test('정적 본문과 FAQ 스키마가 실린다', async ({ page }) => {
    ...
    const chars = (await seo.innerText()).length;
  ...
  test('모바일 라이트 셸에서 글자가 흰색이 아니다', async ({ page }, testInfo) => {
    test.skip(
      testInfo.project.name === 'Desktop Chrome',
      '데스크톱은 어두운 배경이라 흰 글씨가 정답이다 — 라이트 셸에서만 의미 있는 검사',
    );
    await page.goto('/planner');
    await expect(page.locator('.planner-mobile-ai')).toBeVisible();

    const targets = [
      { sel: '.planner-seo-info li', what: '색인 본문' },
      { sel: '.planner-seo-info h2', what: '색인 본문 제목' },
      { sel: '.ai-planner-pricing-note span', what: '가격 안내' },
    ];
