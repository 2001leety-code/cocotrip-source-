...
type FoodRow = { name?: string; nameEn?: string; city?: string; address?: string; [key: string]: unknown };

const __dirname = dirname(fileURLToPath(import.meta.url));
const foodIndex = JSON.parse(readFileSync(join(__dirname, '../../api/_food_index.json'), 'utf-8')) as FoodRow[];
...
describe('food-helper row integrity — real known-wrong rows (#3)', () => {
  it('a Busan-tagged row whose real address is in Gyeonggi (Everhalal near Everland) is excluded', () => {
    const row = foodIndex.find((r) => /Everhalal/i.test(r.nameEn || r.name || '') && r.city === 'busan');
  ...
  it('every real Busan-city row that survives the gate actually has a Busan address', () => {
    const busanRows = foodIndex.filter((r) => r.city === 'busan');
    const valid = busanRows.filter((r) => isValidExactCityFoodRow(r, 'busan'));
    const mismatched = valid.filter((r) => !/부산|busan/i.test(r.address || ''));
    expect(mismatched).toEqual([]);
  });

  it('Incheon retail/mart rows (화수할인마트, 뉴월드마트) are excluded from exact-city candidates', () => {
    const marts = foodIndex.filter((r) => r.city === 'incheon' && /마트|슈퍼/.test(r.name || ''));
    ...
    const candidateNames = candidates.map((r) => r.nameEn || r.name);
    for (const row of marts) {
