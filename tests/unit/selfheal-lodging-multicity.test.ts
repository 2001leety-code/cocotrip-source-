...
type Stop = { category: string; name: string; display_name?: string; start_time?: string; stay_min?: number; order?: number };
type Itinerary = { days: Array<{ day: number; city?: string; stops: Stop[] }> };
...
describe('selfHealLodgingBookend — 다도시 lodging 도시 폴백 (plan 4d214e83)', () => {
  it('부산 day 의 합성 lodging 은 첫 도시(서울) 호텔이 아니라 부산 zone (지리 파탄 차단)', () => {
    ...
    const seoulLodge = itinerary.days[0].stops.find((s: Stop) => s.category === 'lodging')!;
    const busanLodge = itinerary.days[1].stops.find((s: Stop) => s.category === 'lodging')!;
  ...
  it('단도시(전 day seoul) plan 은 기존 동작 — 전 day 사용자 호텔 (회귀 0)', () => {
    ...
    expect(itinerary.days[0].stops.find((s: Stop) => s.category === 'lodging')!.name).toContain('서울');
    expect(itinerary.days[1].stops.find((s: Stop) => s.category === 'lodging')!.name).toContain('서울');
  });
  ...
  it('첫 도시가 부산이고 다음이 서울인 역순 다도시도 도시별 폴백', () => {
    ...
    const busanLodge = itinerary.days[0].stops.find((s: Stop) => s.category === 'lodging')!;
    const seoulLodge = itinerary.days[1].stops.find((s: Stop) => s.category === 'lodging')!;
