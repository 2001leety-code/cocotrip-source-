...
describe('PR #461 X-H2 — source-level invariants (retry sites use retryModel)', () => {
  ...
  it('runGeminiPipeline builds BOTH model and retryModel', () => {
    // P171/P172 (2026-05-23/24): buildModel(apiKey, temperatureOverride, { isAdminBypass, identifierForBucketing })
    expect(src).toMatch(/const\s+model\s*=\s*buildModel\(apiKey,\s*undefined,\s*\{\s*isAdminBypass,\s*identifierForBucketing\s*\}\s*\)\s*;/);
    expect(src).toMatch(/const\s+retryModel\s*=\s*buildModel\(apiKey,\s*RETRY_TEMPERATURE,\s*\{\s*isAdminBypass,\s*identifierForBucketing\s*\}\s*\)\s*;/);
  });
