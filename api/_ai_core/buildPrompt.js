...
- 🔴 **B-16 STRICT ENFORCEMENT (2026-05-12, P202 강화 2026-05-26, P276+P277 v2 minimal 2026-05-29)**: \`departure_guide\`, \`departure_guide.airport\`, \`arrival_guide.airport\` 각 필드 **모두 명시 의무**. 누락 시 backend validator 가 plan 을 거부하고 retry. ALWAYS include the top-level \`departure_guide\` object AND set \`departure_guide.airport\` (예: "ICN T1", "GMP", "PUS"). Likewise \`arrival_guide.airport\` 는 \`arrival_airport != "already_in_korea"\` 일 때 반드시 채워야 한다. PDF 첫/마지막 페이지가 빈 페이지가 되는 사용자 신고 사유. **P276+P277 추가**: \`arrival_guide.steps\` (Immigration / SIM / T-money / Currency / Get to Hotel, 5개) + \`daily_budget_summary\` (Day 수만큼 element, 6 field) 통째 누락 절대 금지 — backend self_heal 발동 시 generic skeleton 노출.
