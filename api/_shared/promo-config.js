/**
 * 프로모 배너 설정 — Firestore admin_config/promo_banner 읽기/쓰기.
 * 어드민이 배포 없이 배너 문구·CTA·마감일·켜끄기를 직접 관리.
 *
 * 안전 설계:
 *   - fail-safe: 읽기 실패/문서 미존재 → DEFAULT_PROMO_CONFIG 반환 (throw 금지).
 *   - setPromoConfig: 화이트리스트 필드만 허용 + 타입 검증. 임의 키 쓰기 금지.
 *   - Firestore rules: admin_config/promo_banner client write false (서버 경유만).
