...
import { buildAdminCors, buildAdminJsonCors, isAdminCorsOriginAllowed } from './cors.js';
import { prepareApprovedExternalInboxReply, validateExternalInboxReplyRequest } from './external-inbox-reply-policy.js';
import { EXTERNAL_INBOX_REPLY_WORKFLOWS, EXTERNAL_INBOX_DRAFT_RETENTION_MS, prepareExternalInboxReplyDraft,
  approveExternalInboxReplyDraft, dispatchExternalInboxReply, validExternalInboxReplyWorkflowRecord } from './external-inbox-reply-workflow.js';
...
export function validExternalInboxReplyAction(input, channel) {
  if (!['email', 'whatsapp'].includes(channel) || !plain(input)
    || !validateExternalInboxReplyRequest(input.request).ok || input.request.channel !== channel) return false;
...
function bodyInput(req, channel) {
  ...
  try {
    ...
    return validExternalInboxReplyAction(input, channel) ? input : null;
  } catch { return null; }
}

async function defaultLoadDb() {
  const { initAdminDb } = await import('./firebase-admin.js');
  return initAdminDb('admin-external-inbox-reply');
}
...
async function loadView({ db, id, actorUid, resolveEnvelope, replyConfig, now, channel }) {
  const source = hash(['external-inbox-reply.source.v1', channel, id]);
  ...
  return db.runTransaction(async tx => {
    ...
    const envelope = await resolveEnvelope(tx, { messageId: id, channel });
    const at = now();
    ...
    const eligibility = prepareApprovedExternalInboxReply({ envelope, actorUid, nowMs: at,
      flags: { replyEnabled: true }, humanApproved: false, request: { messageId: id, channel,
    ...
    const canCompose = (!record || editable.includes(record.status))
      && (eligible || (channel === 'email' && eligibility.code === 'REPLY_CONTEXT_MISSING'));
...
export function createAdminExternalInboxReplyHandler({ channel, readInboxConfig, readReplyConfig,
  ...
  return async (req, res) => {
    ...
    try {
      ...
      const input = req.method === 'POST' ? bodyInput(req, channel) : null;
      ...
      const inboxConfig = readInboxConfig(env, at);
      const replyConfig = readReplyConfig(env, inboxConfig);
      ...
      if (req.method === 'GET') {
        const data = await loadView({ db, id, actorUid: auth.uid, resolveEnvelope, replyConfig, now, channel });
