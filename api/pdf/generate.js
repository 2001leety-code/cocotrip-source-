...
// 2026-07-28: 손님이 실제로 받는 PDF 가 서버판인데 식당(할랄/비건)·예산표·번역이 없었다.
//   섹션 빌더는 _shared/pdf-sections.js 로 분리(프론트 정본 pdfGenerator.ts 의 미러).
import {
  buildRecommendedRestaurantsSection, buildBudgetTableHtml, pdfCategoryBar, pdfLabels,
} from '../_shared/pdf-sections.js';
...
// (2026-07-28 완료) 식당·예산·라벨 섹션은 api/_shared/pdf-sections.js 로 분리됨.
// export: 테스트가 실제 렌더 결과를 검사할 수 있게 (핸들러는 그대로 이 함수를 쓴다).
export function buildPlanHtml(plan) {
  ...
  const css = `
    ...
    .stop { padding: 12px; border: 1px solid #e0e0e0; border-left-width: 4px; border-radius: 8px; margin-top: 8px; background: #f8f9fc; page-break-inside: avoid; break-inside: avoid; }
    /* 추천 식당 카드가 6장마다 페이지를 넘기도록 (클라 템플릿과 동일 규약) */
  ...
  // 손님 언어 — 일/중/한 손님도 영어 PDF 를 받던 문제(2026-07-28).
  const lang = String(input.language || plan.language || 'en');
  const L = pdfLabels(lang);

  let body = `<div class="container">`;
  body += `<div class="header">
    <h1>${escapeHtml(it.tour_title || 'Your Korea Itinerary')}</h1>
    <p class="meta">${escapeHtml(input.startDate || '')} | ${input.adults || input.pax || '-'} ${escapeHtml(L.pax)}</p>
    <p class="meta">${escapeHtml(L.generatedBy)}</p>
  </div>`;
  ...
  if (arrival && arrival.steps) {
    ...
    body += `<div class="guide-card" style="border-color:${C.accent};">
      <h3 style="color:${C.accent};">✈ ${escapeHtml(L.arrivalGuide)} — ${escapeHtml(arrival.airport || '')}</h3>`;
    ...
    if (rec && rec.key !== 'cocotrip_charter' && route) {
      ...
      body += `<div class="guide-hero" style="background:linear-gradient(135deg,rgba(124,92,252,0.10),rgba(234,83,126,0.06));border:1px solid ${C.accent};">
        <p style="font-size:9px;font-weight:700;color:#FBBC05;text-transform:uppercase;letter-spacing:1px;margin:0 0 6px;">★ ${escapeHtml(L.recommended)}</p>
  ...
  for (let di = 0; di < days.length; di++) {
    const day = days[di];
    body += `<div class="day"><div class="day-title">${escapeHtml(L.day)} ${day.day || di + 1}: ${escapeHtml(day.theme || '')}</div>`;
    const stops = day.stops || [];
    for (const stop of stops) {
      const name = stop.display_name || stop.name || stop.name_ko || stop.name_en || '';
      // 카테고리 색 바 — 웹/클라 PDF 와 같은 팔레트라 한눈에 종류가 구분된다.
      body += `<div class="stop" style="border-left-color:${pdfCategoryBar(stop.category)};">
        ...
        ${stop.tip ? `<div class="stop-tip">${escapeHtml(L.tip)}: ${escapeHtml(stop.tip)}</div>` : ''}
      </div>`;
    }
    body += `</div>`;
  }
  ...
  // ── 예산표 ────────────────────────────────────────────────────────────────
  body += buildBudgetTableHtml(it.daily_budget_summary || [], lang, C);

  // ── P193 SAFETY: 추천 식당(할랄/비건) ──────────────────────────────────────
  // 무슬림/비건 손님이 받는 PDF 에 식이제한 식당이 0건이 되지 않게 한다.
  body += buildRecommendedRestaurantsSection(it, lang, C);
  ...
  if (departure) {
    ...
    body += `<div class="guide-card" style="border-color:${C.pink};">
      <h3 style="color:${C.pink};">✈ ${escapeHtml(L.departureGuide)} — ${escapeHtml(departure.airport || '')}</h3>`;
    ...
    if (depRoute) {
      ...
      body += `<div class="guide-hero" style="background:linear-gradient(135deg,rgba(234,83,126,0.10),rgba(251,146,60,0.06));border:1px solid ${C.pink};">
        <p style="font-size:12px;font-weight:700;color:${C.pink};margin:0 0 2px;">${escapeHtml(L.toAirport)}</p>
    ...
    if (departure.luggage_storage && departure.luggage_storage.available) {
      body += `<p style="font-size:10px;color:${C.sub};margin:6px 0 0;"><strong>🧳 ${escapeHtml(L.luggageStorage)}:</strong> ${escapeHtml(departure.luggage_storage.location || '')}</p>`;
    }
    if (departure.tax_refund) {
      const threshold = departure.tax_refund.threshold_krw || 30000;
      body += `<p style="font-size:10px;color:${C.sub};margin:6px 0 0;"><strong>💰 ${escapeHtml(L.taxRefund)}:</strong> ${escapeHtml(departure.tax_refund.location || '')} (${escapeHtml(L.minPurchase)} ${formatKRW(threshold)})</p>`;
    }
    if (departure.last_minute_shopping) {
      body += `<p style="font-size:10px;color:${C.sub};margin:6px 0 0;"><strong>🛒 ${escapeHtml(L.lastMinuteShopping)}:</strong> ${escapeHtml(String(departure.last_minute_shopping))}</p>`;
    }
