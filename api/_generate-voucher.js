 ...
 *
 * pdfkit + qrcode 사용 (표준 Helvetica 폰트, 파일 시스템 불필요)
 * booking-processor.js에서 호출되는 유틸리티 (HTTP handler 없음)
 *
...
/**
 * PDF 바우처 생성
 * @param {object} booking - 예약 데이터 (booking-processor의 booking 객체)
 * @returns {Promise<Buffer>} PDF 바이너리 버퍼
 */
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      try {
        ...
        console.warn('[generate-voucher] QR 생성 실패 (계속 진행):', qrErr.message);
      }
