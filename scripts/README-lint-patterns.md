...
- 커밋의 글자 깨짐 검사는 기존 nullish 문법도 오인할 수 있다. 검사를 끄거나 기존의 빈 문자열·false 의미를 바꾸지 말고, 필요한 표현만 같은 의미의 조건식으로 정리한 뒤 해당 회귀 검사를 실행한다.
- Firebase 의존 화면의 실제 실행 검사는 외부 어댑터 mock을 등록한 다음 Vitest의 `vi.importActual`로 실제 컴포넌트를 평가하고, Firebase 환경변수가 없는 상태에서도 테스트 수집·실행이 되는지 확인한다. 하위 탭의 간접 import도 포함해 전체 unit을 키 없는 환경으로 실행한다. 컴포넌트 자체를 mock하거나 보호 검사를 끄지 않는다.
- effect의 상태 초기화를 옮길 때 최초 표시·로그인 전환·인증 재확인을 각각 확인한다. 이전 입력을 현재 값으로 초기화하면 처음의 자동 입력/보정이 빠질 수 있고, 두 조회의 갱신 조건을 합치면 요청 없이 로딩만 남을 수 있다. 날짜 기준도 처음 한 번 고정하지 말고 원래 데이터 갱신 시점과 맞춘다.
- Workflow 전용 `cross-surface-audit.js`는 최상위 return을 쓰므로 해당 파일의 parser 문법만 설정한다. 파일 제외/규칙 해제 없이 `workflow-eslint-grammar.test.ts`로 원본·잘못된 문법·일반 모듈을 구분한다. 문법 검사 통과는 Workflow 실행 성공이 아니다.
- `npm run lint` 는 저장소 전체 ESLint이고, 이 문서의 mistake-pattern lint 는 오답 패턴만 검사한다.
- `npm run build` 의 `tsconfig.app` 대상은 `src` 중심이라 `tests` 는 타입 검사되지 않는다.
- 따라서 `any` 를 `unknown` 으로 일괄 치환하면 호출·전개·중첩 속성 타입 오류를 숨길 수 있으니 실제 계약을 확인한다.
- 잘못된 테스트 fixture와 입력·검증 동작은 린트 통과를 위해 바꾸거나 약화하지 않는다.
- 상태 갱신 경고를 타이머·마이크로태스크로 숨기지 않는다. 초기값만 옮겨 props 변경 시 갱신을 잃거나, 컴포넌트 전체를 다시 마운트해 작성 중 입력을 지우지 않도록 실제 전환을 검사한다.
- 비동기 화면은 입력 삭제·응답 역전·A→B→A 전환을 검사한다. 이전 요청의 완료 표시가 새 요청에 적용되지 않아야 한다.
- 자동 조회와 수동 새로고침의 조건을 섞지 않는다. 저장 경과 표시처럼 시간이 입력인 화면은 가짜 시계로 시간이 지난 뒤의 표시까지 확인한다.
- ESLint는 Git의 ignore 목록을 자동 적용하지 않는다. 수정 전 소스 백업은 저장소 밖에 두어 중복 진단을 만들지 않는다.
