...
- Firebase 의존 화면의 실제 실행 검사는 외부 어댑터 mock을 등록한 다음 Vitest의 `vi.importActual`로 실제 컴포넌트를 평가하고, Firebase 환경변수가 없는 상태에서도 테스트 수집·실행이 되는지 확인한다. 컴포넌트 자체를 mock하거나 보호 검사를 끄지 않는다.
