 ...
 * 거리추가는 차종 무관 정액 / 기본·오버타임은 차종배수(staria 1.0 / sprinter 2.0).
 */

const TOUR_BASE_9H_KRW = 405_000;
const TOUR_OVERTIME_HOURLY_KRW = 54_000;
const TOUR_VEHICLE_MULT: Record<string, number> = { staria: 1.0, sprinter: 2.0 };
...
export function calcTourQuote(
  { km = 0, vehicle, couponPct = 5 }: { km?: number; vehicle: string; couponPct?: number },
): TourQuote | null {
