...
export function HotelAd({ region }: HotelAdProps) {
  ...
  return (
    <AffiliateCard payload={{ product: 'hotel', placement: 'plan_pretrip', language, city: region.toLowerCase() }}
      className="mb-6 rounded-2xl overflow-hidden border border-blue-500/20" style={{ background: 'linear-gradient(135deg, rgba(59,130,246,0.08), rgba(6,182,212,0.05))' }}>
      <div className="px-5 py-4 flex items-center gap-3">
        ...
        <div className="flex-1">
          ...
          <p className="text-[14px] text-white/50 mt-0.5">{(p.adHotelSub || 'Best rates for {region} hotels').replace('{region}', region)}</p>
        </div>
      </div>
      <div className="px-5 pb-4">
        {links.map((lk: { provider: string; url: string; label: string; color?: string }) => (
          <a key={lk.provider} href={lk.url} target="_blank" rel="noopener noreferrer sponsored" onClick={() => trackAffiliateClick({ product: 'hotel', placement: 'plan_pretrip', language, city: region.toLowerCase() })}
