...
// 실데이터(plan.itinerary.days[].stops)에서만 파생: 사진·이름·개인화 사유.
...
export function TripHighlights({ plan, language }: { plan: PlanDocument; language: Language }) {
  ...
  // 우선순위: 사진 있음 > verified > 첫 등장. 최대 4일.
  const rankStop = (s: PlanStop) => (s.photo_ref ? 2 : 0) + (s.verified ? 1 : 0);
    ...
    <section className="plan-mobile-summary mx-auto max-w-[430px] px-4 pt-1 pb-2">
      ...
      <div className="flex gap-2.5 overflow-x-auto pb-1 scrollbar-hide">
        {picks.map((s, i) => (
          ...
          >
            <div className="relative h-[84px] w-full bg-[#efeafc]">
              {s.photo_ref && (
                <img
                  src={`/api/place-photo?ref=${encodeURIComponent(s.photo_ref)}&w=400`}
                  alt=""
                  loading="lazy"
