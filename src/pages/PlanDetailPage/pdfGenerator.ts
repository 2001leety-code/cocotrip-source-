...
export interface PdfUiDict {
  ...
  budgetDay?: string;
  /** 행 값 템플릿 — "{n}" 자리표시자 포함 (ja/zh 어순 때문에 열 제목과 별도 키). */
  budgetDayN?: string;
...
export async function generatePDF(
  ...
  const L = {
    ...
    // day = 열 제목(숫자 없는 명사). dayN = 행 값 템플릿("{n}" 포함) — 화면 BudgetTable 과 동일 SSOT.
    day: uiDict?.budgetDay || 'Day',
    dayN: uiDict?.budgetDayN,
    ...
    (day.stops || []).forEach((stop: PlanStop, si: number) => {
      ...
      if (stop.transit_from_prev) {
        ...
        const walkNote = (t.method === 'walk' && (t.est_min || 0) <= 15)
          ? `<p style="font-size:9px;color:#10b981;margin:2px 0 0;font-style:italic;">${transitDict?.walkFasterNote || '🚶 이 거리는 지하철보다 도보가 빠릅니다 (대기·환승 포함)'}</p>`
          : '';
