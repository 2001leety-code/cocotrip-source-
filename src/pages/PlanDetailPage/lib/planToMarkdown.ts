...
import type { PlanDocument, RecommendedRestaurant } from '../types';
...
export function planToMarkdown(
  ...
  for (const day of (it.days || [])) {
    ...
    for (const stop of (day.stops || [])) {
      const icon = CATEGORY_ICON[stop.category as string] || '📍';
  ...
  if (Array.isArray(it.daily_budget_summary) && it.daily_budget_summary.length > 0) {
    ...
    out.push('|-----|------|---------|-------|-------|');
    it.daily_budget_summary.forEach((b, i) => {
  ...
  if (rr && typeof rr === 'object') {
    ...
    for (const [bucket, list] of Object.entries(rr as Record<string, unknown[]>)) {
      ...
      for (const r of list.slice(0, 10)) {
        const rr = r as RecommendedRestaurant & { display_name?: string };
...
export function downloadPlanAsMarkdown(
  ...
  const filename = opts.filename
    || `cocotrip-plan-${plan?.planId?.slice(0, 8) || 'export'}.md`;
