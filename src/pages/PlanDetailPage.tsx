      ...
      (day.stops || []).forEach((stop: any) => {
        ...
        if (stop.transit_from_prev) {
          ...
          html += `<div style="margin:4px 0 6px 16px;padding:6px 12px;background:${C.transitBg};border-left:3px solid ${C.accent};border-radius:4px;">
            <p style="font-size:10px;color:${C.accent};font-weight:700;margin:0;">🚇 ${t.method} — ${t.est_min || '?'}min${t.est_fare_krw > 0 ? ` (${formatKRW(t.est_fare_krw)})` : ''}${t.source === 'odsay' ? ' [실시간]' : ''}</p>
        ...
        html += `<div style="background:${C.cardBg};border:1px solid ${C.border};border-radius:8px;padding:12px;margin-bottom:6px;">
          <div style="display:flex;justify-content:space-between;align-items:flex-start;">
            <div>
              <p style="font-size:13px;font-weight:700;color:${C.heading};margin:0;"><span style="color:${C.accent};font-size:12px;">${stop.start_time || ''}</span> — ${stop.name_en || stop.name_ko || ''}</p>
              ${stop.name_ko && stop.name_en ? `<p style="font-size:10px;color:${C.muted};margin:2px 0 0;">${stop.name_ko}</p>` : ''}
            </div>
