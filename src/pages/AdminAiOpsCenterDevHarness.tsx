...
import type {
  WebchatDetail,
  WebchatOverview,
} from "@/lib/adminWebchatInboxContract";
import {
...
function createPreviewData(now: number, search: string): OpsCenterData {
  ...
  const dateAt = (offset: number) =>
    new Date(dayStart + offset * DAY_MS + 9 * HOUR_MS)
      .toISOString()
      .slice(0, 10);
  const NOW = now;
  const gate = {
    ready: false,
    reason: "DISABLED",
    delivery: "not-verified" as const,
  };
  ...
  const previewData: OpsCenterData = {
    ...
    channelResponseReadiness: {
      ...
      channels: (
        ...
        channel,
        implementation: "synthetic-preview",
        supported: channel !== "instagram" && channel !== "tiktok",
        intake: {
          status:
            channel === "webform" || channel === "webchat"
              ? "implemented"
              : channel === "email" || channel === "whatsapp"
                ? "not_ready"
                : "not_implemented",
          detail: "synthetic-preview",
        },
        autoAck: gate,
        ownerPush: gate,
      })),
    ...
    workItems: [
      {
        workItemId: "payment_reviews:ORDER-REVIEW",
        type: "payment_review",
        sourceSystem: "payment_reviews",
        sourceRecordId: "ORDER-REVIEW",
        title: "결제 격리 확인 · ORDER-REVIEW",
      ...
      {
        workItemId: "bookings:CT-TODAY",
        type: "reservation",
        sourceSystem: "bookings",
        sourceRecordId: "CT-TODAY",
        title: "배차 상태 확인 · CT-TODAY",
        status: "CONFIRMED",
        priority: "P0",
        nextAction: "배차 상태 확인",
        actionRequired: true,
        ageHours: 8,
        eventDate: dateAt(0),
        createdAtMs: NOW - 8 * HOUR_MS,
        deepLink: "/admin/calendar",
      },
      {
        workItemId: "charter_inquiries:INQ-OLD",
        type: "inquiry",
        sourceSystem: "charter_inquiries",
        sourceRecordId: "INQ-OLD",
        title: "맞춤 투어 문의 · INQ-OLD",
        status: "pending",
        priority: "P1",
        nextAction: "최종 답변 검토",
        actionRequired: true,
        ageHours: 29,
        eventDate: dateAt(4),
        createdAtMs: NOW - 29 * HOUR_MS,
        deepLink: "/admin/claims",
      },
      {
        workItemId: "automation:email_retry",
        type: "automation",
        sourceSystem: "email_retry",
        sourceRecordId: "email_retry",
        title: "고객 이메일 · 2건",
        status: "attention",
        priority: "P1",
        nextAction: "수동 처리 필요",
    ...
    reservations: [
      ...
      {
        workItemId: "pending_bookings:CT-PENDING",
        sourceSystem: "pending_bookings",
        sourceLabel: "입금 대기",
        sourceRecordId: "CT-PENDING",
        bookingRef: "PREVIEW-CT-PENDING",
        customerIdentityVerified: false,
        tripAt: dateAt(2),
        tripAtMs: dayStart + 2 * DAY_MS,
        reservationStatus: "AWAITING_VERIFICATION",
      ...
      {
        workItemId: "mood_bookings:MOOD-01",
        sourceSystem: "mood_bookings",
        sourceLabel: "MOOD",
        sourceRecordId: "MOOD-01",
        bookingRef: "MOOD-01",
        customerIdentityVerified: false,
        tripAt: dateAt(3),
        tripAtMs: dayStart + 3 * DAY_MS,
        reservationStatus: "confirmed",
        paymentStatus: "confirmed",
        dispatchStatus: "unknown",
        replyStatus: "not_applicable",
        ...
        updatedAtMs: NOW - 4 * HOUR_MS,
        createdAtMs: NOW - 4 * HOUR_MS,
        deepLink: "/admin/all-bookings",
        label: "vehicle",
        isTest: true,
      },
    ],
    ...
    automation: [
      ...
      },
      {
        ...
        deepLink: "https://github.com/2001leety-code/cocotrip-source-/actions",
      },
    ],
    sources: [
      {
  ...
  if (
    ["ok", "partial", "empty", "unknown", "zero", "no-today"].includes(
      usageState || "",
    )
  ) {
    ...
    previewData.recordedAiUsage = {
      ...
      todayStart: new Date(dayStart).toISOString(),
      queryLimit: 500,
      recordedCostUsd: noRecords ? null : usageState === "zero" ? 0 : 1.234567,
      todayRecordedCostUsd:
        noRecords || usageState === "no-today"
          ? null
          : usageState === "zero"
            ? 0
            : 0.123456,
      ...
      todayRecordCount:
        usageState === "unknown"
          ...
          : noRecords || usageState === "no-today"
            ? 0
            : 2,
      excludedCount: usageState === "unknown" ? null : 0,
      limitReached: usageState === "partial",
      latestRecordAt: noRecords
        ? null
        : new Date(
            usageState === "no-today" ? dayStart - 1000 : NOW - 1000,
          ).toISOString(),
    };
  }
  ...
  if (
    ...
    dispatchState === "configuration_required" ||
    dispatchState === "unknown"
  ) {
    previewData.ownerDispatchReadiness = {
      state: dispatchState,
  ...
  previewData.workItems.push(
    {
      ...
      title: `${pending.nextAction} · ${pending.bookingRef}`,
      status: pending.reservationStatus,
      priority: pending.priority,
      nextAction: pending.nextAction,
      actionRequired: true,
      ageHours: 2,
      eventDate: pending.tripAt,
      createdAtMs: pending.createdAtMs,
      deepLink: pending.deepLink,
    },
    {
      workItemId: "automation:planner_retry",
      type: "automation",
      sourceSystem: "planner_retry",
      sourceRecordId: "planner_retry",
      title: "AI 플래너 생성 · 1건",
      status: "retrying",
      priority: "P2",
      nextAction: "자동 재시도 확인",
      actionRequired: true,
      ageHours: 0,
      eventDate: "",
      createdAtMs: 0,
  ...
  previewData.inboxItems = previewData.workItems.filter(
  ...
  if (params.get("work-list") === "long") {
    for (let index = previewData.workItems.length; index < 23; index += 1) {
      previewData.workItems.push({
        workItemId: `decision_queue:PREVIEW-${index + 1}`,
        type: "decision",
        sourceSystem: "decision_queue",
        sourceRecordId: `PREVIEW-${index + 1}`,
        title: `미리보기 확인 업무 ${index + 1}`,
        status: "pending",
        priority: "P2",
        nextAction: "내용 확인",
        actionRequired: true,
  ...
  if (params.get("reservations") === "long") {
    ...
    for (let index = previewData.reservations.length; index < 65; index += 1) {
      ...
      previewData.reservations.push({
        ...template,
        workItemId: `mood_bookings:PREVIEW-${index + 1}`,
        sourceRecordId: `PREVIEW-${index + 1}`,
        bookingRef: `PREVIEW-${index + 1}`,
        label: `미리보기 예약 ${index + 1}`,
        tripAt: dateAt(offset),
        tripAtMs: dayStart + offset * DAY_MS,
      });
    }
  }

  if (params.get("email-state") === "empty") {
    previewData.workItems = previewData.workItems.filter(
      (item) => item.sourceSystem !== "email_retry",
    );
    previewData.automation = previewData.automation.map((item) =>
      item.key === "email_retry"
        ? {
            ...item,
            status: "ok",
            pending: 0,
  ...
  if (
    params.get("scenario") === "empty" ||
    params.get("scenario") === "partial-empty"
  ) {
    ...
    previewData.partialErrors =
      params.get("scenario") === "partial-empty"
        ? ["bookings", "charter_inquiries", "cs_tickets"]
        : [];
  }

  previewData.sources = previewData.sources.map((source) => {
    const count =
      source.key === "charter_inquiries"
        ? previewData.inboxItems.length
        : source.key === "pending_email_retries"
          ? previewData.automation.find((item) => item.key === "email_retry")
              ?.count || 0
          : previewData.reservations.filter(
              (item) => item.sourceSystem === source.key,
            ).length;
    return {
      ...source,
      ok: !previewData.partialErrors.includes(source.key),
      count,
    };
  });
  // Deliberately omit partialErrors here to exercise the independent source flag.
  if (params.get("query-range") === "limited") {
    previewData.sources[0] = {
      ...previewData.sources[0],
  ...
  previewData.summary = {
    actionRequired: previewData.workItems.filter((item) => item.actionRequired)
      .length,
    urgent: previewData.workItems.filter(
      (item) => item.priority === "P0" || item.priority === "P1",
    ).length,
    todayReservations: previewData.reservations.filter(
      (item) => item.tripAtMs >= dayStart && item.tripAtMs < dayStart + DAY_MS,
    ).length,
    upcoming7d: previewData.reservations.filter(
      (item) =>
        item.tripAtMs >= dayStart && item.tripAtMs < dayStart + 8 * DAY_MS,
    ).length,
    openInquiries: previewData.inboxItems.filter(
      (item) => item.type === "inquiry" && item.actionRequired,
    ).length,
    openCs: previewData.inboxItems.filter(
      (item) => item.type === "cs" && item.actionRequired,
    ).length,
    paymentReviews: previewData.workItems.filter(
      (item) => item.type === "payment_review",
    ).length,
    automationAttention:
      previewData.automation.filter(
        (item) => item.status === "attention" || item.status === "retrying",
      ).length + previewData.partialErrors.length,
  };
  return previewData;
}
...
export default function AdminAiOpsCenterDevHarness() {
  ...
  const previewExternalInboxRetentionAction = useCallback(
    async (input: {
      ...
      if (
        input.messageId !== "1".padStart(64, "0") ||
        input.expectedRevision !== retentionState.revision ||
        params.get("retention-conflict") === "1"
      )
        throw new Error("CASE_TRANSITION_CONFLICT");
      const current = retentionState;
      const next: ExternalInboxRetention =
        input.action === "close"
          ? {
              ...current,
              status: "closed",
      ...
      if (params.get("retention-late") === "1")
  ...
  const sessions: WebchatOverview["sessions"] = ["en", "ko"].map(
    (lang, index) => ({
      ...
      lastMessageAtMs: now - (index + 1) * 60_000,
      lastMessageFrom: "customer",
      ownerType: "guest",
      language: lang === "ko" ? "ko" : "en",
    }),
  );
  const details: Record<string, WebchatDetail> = Object.fromEntries(
    sessions.map((session) => [
      session.sessionId,
      {
        generatedAtMs: now,
        session,
        messagesPossiblyTruncated: false,
        messages: [
          {
            ...
            text:
              session.language === "en"
                ? "Can I ask a question about my travel plan? (Synthetic test)"
                : "여행 일정에 관해 문의하고 싶어요. (합성 시험 자료)",
            language: session.language,
            truncated: false,
            ts: session.lastMessageAtMs,
          },
        ],
      },
    ]),
  ...
  const checks: OperationalChecksData = {
    ...
    checks: operationalWorkflowNames.map((workflow, index) => {
      const run = {
        ...
        conclusion: index === 2 ? null : index === 1 ? "failure" : "success",
        createdAtMs: now - HOUR_MS,
        updatedAtMs: now - 60_000,
        url: `https://github.com/2001leety-code/cocotrip-source-/actions/runs/${index + 1}`,
      };
      return {
        key: workflow.slice(0, -4),
        workflow,
        latestRun: run,
        lastSuccessfulRun: run.conclusion === "success" ? run : null,
        freshness: index === 3 ? "stale" : "fresh",
        checkedAtMs: now - (index === 3 ? HOUR_MS : 0),
        runHealth: index === 1 ? "failed" : index === 2 ? "running" : "ok",
        maxAgeMs: (index === 0 ? 4 : 9) * DAY_MS,
