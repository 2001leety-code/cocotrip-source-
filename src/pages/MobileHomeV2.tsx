...
export default function MobileHomeV2() {
  ...
  return (
    <div className="min-h-screen bg-[#0a0b14] text-white pb-24 max-w-md mx-auto">
      ...
      {/* ── Hero (텍스트 + CTA, 깔끔) ── */}
      <section className="relative overflow-hidden px-5 pt-4 pb-6">
        ...
        <div className="relative">
          ...
          <Link
            to="/planner"
            className="mt-5 inline-flex items-center gap-1.5 rounded-full bg-gradient-to-r from-purple-500 to-pink-500 px-6 py-3 text-sm font-semibold shadow-lg shadow-purple-500/30 active:scale-95 transition-transform"
          >
        ...
        <div className="grid grid-cols-5 gap-1">
          {CATEGORIES.map((c) => (
            <Link key={c.label} to={c.to} className="flex flex-col items-center gap-1.5 py-2 active:opacity-70">
              ...
              >
                <c.icon size={21} className={c.accent ? 'text-white' : 'text-purple-300'} />
              </span>
              <span className="text-center text-[11px] leading-tight text-white/70">{c.label}</span>
            </Link>
          ))}
        </div>
      </section>
      ...
      <section className="px-5 pt-6">
        <div className="mb-3 flex items-center justify-between">
          <h2 className="text-lg font-bold">Explore Korea</h2>
        ...
        <div className="grid grid-cols-2 gap-3">
          {REGIONS.map((r) => (
            <Link
              key={r.id}
              to={`/region/${r.id}`}
              className="relative h-32 overflow-hidden rounded-2xl active:scale-[0.98] transition-transform"
            >
