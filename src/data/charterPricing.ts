...
// 차터 전체 USD 청구·표시 고정환율. 백 createPaypalOrder SPEC.charter_usd_fix_rate 와 동일 SSOT.
// priceUSD 표시(카드/영수증/위시리스트)도 이 rate 로 KRW÷rate 산출 → 표시가==청구가(P311). live 환율 변동 무관.
export const CHARTER_USD_FIX_RATE: number = (spec as { charter_usd_fix_rate?: number }).charter_usd_fix_rate || 1350;
  ...
  Object.entries(spec.airport_transfer_prices)
    ...
    .map(([key, raw]) => {
      ...
      return [key, {
        ...
        en: v.name_en,
        priceKRW: v.priceKRW,
        priceUSD: Math.round(v.priceKRW / CHARTER_USD_FIX_RATE), // 고정 1350원/USD 기준 산출 (표시==청구)
