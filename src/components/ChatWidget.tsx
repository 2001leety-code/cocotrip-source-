...
export function ChatWidget({ language, hideTrigger }: ChatWidgetProps) {
  ...
  const { messages, loading, sendMessage, quickShown, guestGated } = useChatSession(language, open);
  ...
  return (
    <>
      {/* ── 채팅 팝업창 ─────────────────────────────────────────
          2026-08-18 퍼널 감사 1번: 로그인 벽 제거 — 게스트도 바로 대화.
          무료 3문답 소진 시 입력창 자리에 로그인 카드(guestGated). */}
      {open && (
        <div
          style={{
            position: 'fixed',
          ...
          {guestGated ? (
            ...
            >
              <div style={{ color: '#fff', fontWeight: 700, fontSize: '14px', marginBottom: '6px' }}>
                {GUEST_GATE_TEXT[language].title}
              </div>
              <p style={{ color: 'rgba(255,255,255,0.6)', fontSize: '12px', marginBottom: '12px' }}>
                {GUEST_GATE_TEXT[language].desc}
              </p>
              <button
                onClick={handleGoogleLogin}
                disabled={authLoading !== null}
                style={{ width: '100%', padding: '10px', borderRadius: '10px', background: '#fff', border: 'none', cursor: 'pointer', fontWeight: 700, fontSize: '13px', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px', opacity: authLoading !== null ? 0.6 : 1 }}
              >
