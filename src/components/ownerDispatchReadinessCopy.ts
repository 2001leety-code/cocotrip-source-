...
const ko = {
  ...
  unavailable: '메일·WhatsApp·웹채팅 알림은 채널별 설정도 필요합니다. API/호스팅 비용 알림은 미지원입니다.',
...
export const ownerDispatchReadinessCopy: Record<Language, Copy> = {
  ...
  en: {
    ...
    unavailable: 'Email, WhatsApp and web-chat alerts also require channel setup. API/hosting cost alerts are not supported.',
  ...
  ja: {
    ...
    unavailable: 'メール・WhatsApp・ウェブチャットの通知にはチャネル設定も必要です。API/ホスティング費用の通知は未対応です。',
    details: '設定範囲と次の手順',
  ...
  zh: {
    ...
    unavailable: '邮件、WhatsApp及网页聊天通知还需渠道配置。API/托管费用通知尚不支持。',
    details: '配置范围与后续步骤',
