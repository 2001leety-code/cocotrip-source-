...
// 여행 스타일(페이스) — 가이드 P7 Custom Tour Inquiry: Relaxed/Balanced/Active.
// (동행유형 Solo/Couple/Family 은 위자드 P3 전용 — 투어 문의는 페이스임. 2026-07-13 레퍼런스 대조 정정.)
// value 영문 고정(텔레그램 규약), 라벨만 4-lang.
const TRAVEL_STYLE_OPTIONS: Array<{ value: string; label: Record<Lang, string> }> = [
  { value: 'Relaxed',  label: { ko: '여유롭게', en: 'Relaxed',  ja: 'ゆったり',   zh: '轻松' } },
  { value: 'Balanced', label: { ko: '균형',     en: 'Balanced', ja: 'バランス',   zh: '均衡' } },
  { value: 'Active',   label: { ko: '활동적',   en: 'Active',   ja: 'アクティブ', zh: '充实' } },
];
...
const TRAVEL_STYLE_LABEL: Record<Lang, string> = { ko: '여행 스타일', en: 'Travel Style', ja: '旅行スタイル', zh: '旅行风格' };
      ...
      >
        ...
        {status === 'success' ? (
          ...
          <form onSubmit={submit} className="p-4 space-y-2.5">
            ...
            {/* 여행 스타일(페이스, 라디오) | 기간(select) — 가이드 P7 Custom Tour */}
            <div className="grid grid-cols-2 gap-2">
              <div>
                <label className="block text-[10px] uppercase tracking-wider text-white/55 mb-1 font-semibold">
                  {TRAVEL_STYLE_LABEL[lang]}
                </label>
                <div className="flex flex-wrap gap-1.5">
                  {TRAVEL_STYLE_OPTIONS.map(opt => {
                    const active = travelStyle === opt.value;
                    return (
                      <button
                        key={opt.value}
                        type="button"
                        onClick={() => setTravelStyle(active ? '' : opt.value)}
                        aria-pressed={active}
